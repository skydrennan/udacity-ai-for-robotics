...
def plan(warehouse, dropzone, todo):
    ...
    while len(todo) > 0:
        ...
        while change:
            ...
            for y in range(len(warehouse)):
                for x in range(len(warehouse[0])):
                    if y == yGoal and x == xGoal and h[y][x] == 99:
                        h[y][x] = 0
                        change = True
                    # the or is because sometimes the starting cell is shown as 'x'
                    if warehouse[y][x] == 0 or (x == x0 and y == y0):
                        for dir in range(len(motions)):
                            x1 = x + motions[dir][1]
                            y1 = y + motions[dir][0]
