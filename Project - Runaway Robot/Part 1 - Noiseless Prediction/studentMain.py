...
def estimate_next_pos(measurement, OTHER = None):
    ...
    if len(OTHER) > 1:
        for i in range(len(OTHER) - 1):
            ...
            y = OTHER[i+1][1] - OTHER[i][1]
            x = OTHER[i+1][0] - OTHER[i][0]
            thetas.append(atan2(y, x))
    else:
        distances = [0]
        thetas = [0]
    ...
    if(len(thetas) > 1):
        for i in range(len(thetas) - 1):
            dtheta.append((thetas[i+1] - thetas[i]) % (2 * pi))
        avgdtheta = sum(dtheta) / float(len(dtheta))
    
    x = measurement[0] + avgdist * cos(thetas[len(thetas)-1] + avgdtheta)
    y = measurement[1] + avgdist * sin(thetas[len(thetas)-1] + avgdtheta)
